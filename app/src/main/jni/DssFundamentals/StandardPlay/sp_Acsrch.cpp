...
if (!w.wk.adaptive_codebook_search_init)
   {
   if ((w.wk.adaptive_codebook_search_acdbk=(double *)calloc(w.wk.maxlag_, sizeof(double))) == NULL)
      {
      printf("\nadaptive_codebook_search init: memory allocation error!\n\n");
      exit(1);
      }
   w.wk.adaptive_codebook_search_init = 1;
   }
if (0 == t && 0 == h && 0 == e && 0 == M && 0 == indexb) {
	free(w.wk.adaptive_codebook_search_acdbk);	w.wk.adaptive_codebook_search_acdbk = 0;
	w.wk.adaptive_codebook_search_init = 0;
	return;
}
...
p1 = w.wk.adaptive_codebook_search_acdbk; p2 = w.wk.adaptive_codebook_search_acdbk + N;
for (i=0; i<w.wk.maxlag_-N; i++) *p1++ = *p2++;
for (i=0; i<N; i++) *p1++ = e[i];

/* --- set pointer to relevant section of adaptive codebook */
acb = w.wk.adaptive_codebook_search_acdbk + w.wk.maxlag_-stoplag;
...
if (accu)
   ...
   for (n=0; n<MIN(lag,N); n++) hcwm[n] = hcbm[n];
   for (; n<N; n++) hcwm[n] = hcbm[n] + hcbm[n-lag];
   ...
   if ((cross=accu) > 0)
      {
      for (accu=0, n=0; n<N; n++) accu += hcwm[n] * hcwm[n];
      energy = accu;
      accu = maxe * cross;
      accu *= cross;
      accu2 = maxc * maxc;
      accu2 *= energy;
      if (accu > accu2) {maxc = cross; maxe = energy; *M = lag;}
      }
   ...
   for (firstpass=1; lag<=stoplag; lag++)
      ...
      for (n=R; n<N; n++)
         hcbm[n] = hcbmprev[n-1];
      for (n=0; n<N; n++) hcwm[n] = hcbm[n];
      if (firstpass)
         {
         for (enmem=0, n=R; n<N; n++) enmem += hcwm[n] * hcwm[n];
         firstpass = 0;
         }
      else
         {
      ...
      if ((cross=accu) > 0)
         {
         for (accu=enmem, n=0; n<R; n++) accu += hcwm[n] * hcwm[n];
         energy = accu;
         accu = maxe * cross;
         accu *= cross;
         accu2 = maxc * maxc;
      ...
      for (n=0; n<N; n++) hcbmprev[n] = hcbm[n];
      }
   }
...
if (w.wk.ltpquant_) beta = quantize(beta, gaqtbl, w.wk.nbga_, indexb);
for (n=1, i=0; i<N; n++)
   for (; i<n*(*M) && i<N; i++)
      e[i] = beta * w.wk.adaptive_codebook_search_acdbk[w.wk.maxlag_-n*(*M)+i];
