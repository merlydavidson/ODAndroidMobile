...
{
printf("\nabsdec - Analysis-by-Synthesis Decoder Simulation      GRUNDIG      %s\n", DATE);
printf("\nusage: absdec infile outfile [-s]\n\n");
exit(0);
}
...
if ((*fpi=fopen(infilename, "rb")) == NULL)
   {
   printf("\nabsdinit: error opening input speech file %s!\n\n", infilename);
   exit(1);
   }
if ((*fpo=fopen(outfilename, "wb")) == NULL)
   {
   printf("\nabsdinit: error opening output speech file %s!\n\n", outfilename);
   exit(1);
   }
#endif
/* --- set parameters --- */
