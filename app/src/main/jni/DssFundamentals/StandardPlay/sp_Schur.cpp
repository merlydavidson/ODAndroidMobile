 ...
 * -------------------------------------------------------------------- *
 *
 *   schur  -  performs Schur recursion (LPC analysis)
 *
 * -------------------------------------------------------------------- *
 *
 *    prototype:         void schur(double *acfs,               i
 *                                  int np,                     i
 *                                  double *k);                 o
 *
 * -------------------------------------------------------------------- *
...
if (acfs[0] == 0)
   for (i=0; i<np; i++) k[i] = 0;
else
   {
   /* --- stabilization --- */
   stabfac = 1./(1.+w.wk.stab_);
   for (i=1; i<=w.wk.np_; i++) acfs[i] *= stabfac;
   /* --- Schur recursion --- */
   /* initialise arrays pp[..] and kk[..] for the recursion */
   for (i=1; i<np; i++) kk[np+1-i] = acfs[i];
   for (i=0; i<=np; i++) pp[i] = acfs[i];
   /* compute reflection coefficients */
   for (n=0; n<np; n++)
      {
      if (pp[0] <= fabs(pp[1]))
         {
         for(i=n; i<np; i++) k[i] = 0;
         return;
         }
      k[n] = -pp[1] / pp[0];
      if (n==np-1) return;
